...
const Login = () => {
  ...
  const onSubmitHandler = async (event) =>{
    event.preventDefault(); {/**prevent the defoult reload */}
    try {
  ...
  return (
      <form onSubmit={onSubmitHandler } className="flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4">
        ...
        {currentState === 'Login' ? '' : <input onChange={(e)=>setName(e.target.value)} value={name} type="text" className="w-full px-3 py-2 border border-gray-800" placeholder="Name" required/> }{/*<-  turnery operator */}
